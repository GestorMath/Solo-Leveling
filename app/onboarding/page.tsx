...
// CRÍTICO: usar createBrowserClient para sincronizar cookies com o middleware
...
export default function OnboardingPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const { error: dbErr } = await supabase.from('players').upsert({
        id: user.id, name: playerName.trim(), class: selectedClass,
        level: 1, xp: 0, gold: 1000, stamina: 20,
        stats: initStats, inventory: [], routines: [], counters: {},
        ...
        {(['name', 'class', 'confirm'] as Step[]).map((s, i) => (
          <div key={s} className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full transition-all duration-300 ${
              ...
              : ['name','class','confirm'].indexOf(step) > i ? 'bg-cyan-800' : 'bg-slate-800'
            }`} />
