  ...
  async function handleSubmit() {
    ...
    try {
      if (tab === 'login') {
        ...
        if (data?.user) {
          // Verifica se tem onboarding completo
        ...
        if (data.session) {
          // Confirmação de email desativada — vai direto pro onboarding
          router.replace('/onboarding')
        } else if (data.user) {
      ...
      <div className="relative z-10 w-full max-w-sm mx-4" style={{ animation: 'cardIn 0.5s both' }}>
          ...
          <div className="inline-flex items-center justify-center mb-4">
            <svg width="52" height="46" viewBox="0 0 48 42">
              <polygon
                ...
                strokeWidth="1.5"
              />
              <text x="24" y="27" textAnchor="middle" fill="rgba(0,255,255,0.9)" fontSize="14" fontWeight="bold">
                S
              </text>
            </svg>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {tab === 'cadastro' && (
              <p className="text-[8px] text-slate-700 mt-1">Mínimo 6 caracteres</p>
            )}
          </div>

          {error && (
            <div className="border border-red-900/60 bg-red-950/30 px-3 py-2.5 text-[9px] text-red-400 font-bold tracking-wider">
              // {error}
            </div>
          )}
          {successMsg && (
