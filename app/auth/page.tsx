...
function GridDot({ x, y, delay }: { x: number; y: number; delay: number }) {
  return (
    <div className="absolute w-[3px] h-[3px] rounded-full bg-cyan-500/20"
      style={{ left: `${x}%`, top: `${y}%`, animationName: 'dotPulse', animationDuration: '3s', animationDelay: `${delay}s`, animationIterationCount: 'infinite', animationTimingFunction: 'ease-in-out' }} />
  )
}
...
export default function AuthPage() {
  ...
  async function handleSubmit() {
    ...
    if (!email.trim() || !password) { setError('CAMPOS OBRIGATÓRIOS — Preencha e-mail e senha.'); return }
    setLoading(true)
    try {
      if (tab === 'login') {
        ...
        if (data?.user) {
          const { data: playerData } = await supabase.from('players').select('class').eq('id', data.user.id).single()
          window.location.href = (!playerData || !playerData.class) ? '/onboarding' : '/Dashboard'
        }
      } else {
        ...
        else { setSuccessMsg('CONTA CRIADA — Verifique seu e-mail para confirmar.'); setLoading(false) }
      }
    } catch (err: any) {
  ...
  async function handleGoogle() {
    ...
    if (authError) { setError('ERRO COM GOOGLE — Tente novamente.'); setLoading(false) }
  }
      ...
      <div className="relative z-10 w-full max-w-sm mx-4" style={{ animation: 'cardIn 0.5s both' }}>
        ...
        <div className="space-y-4">
          ...
          <button onClick={handleGoogle} disabled={loading}
            className="w-full py-3 bg-black border border-slate-800 text-slate-400 font-bold text-[10px] uppercase tracking-widest hover:border-slate-600 flex items-center justify-center gap-2">
