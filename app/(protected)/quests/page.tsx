...
import {
  Shield, BookOpen, Smartphone, Cross, Activity,
  Dumbbell, Utensils, Droplets, Layout, Bed, Check,
} from 'lucide-react'

const TASK_COUNTER_MAP: Record<string, string> = {
  pushups: 'pushups', reading: 'reading', squats: 'squats',
  plank: 'plank', focus: 'focus', meditation: 'meditation',
  water: 'water', bed: 'bed', dishes: 'dishes', organize: 'organize', stretch: 'stretch',
}

const STORAGE_KEY = 'system_quest_status_v2'
...
function loadQuestStatus() {
  try {
    ...
    // Se for de outro dia, retorna null para resetar
...
export default function QuestsPage() {
  ...
  }>({ daily: [], systemCompleted: [] })

  useEffect(() => {
    setIsMounted(true)
    // Carrega do localStorage apenas uma vez na montagem
    const saved = loadQuestStatus()
    if (saved) {
      setCompletedQuests({
        daily: saved.daily ?? [],
        systemCompleted: saved.systemCompleted ?? [],
      })
    }
  }, [])
  ...
  useEffect(() => {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify({
      ...
      systemCompleted: completedQuests.systemCompleted,
    }))
  ...
  if (!isMounted || !system) {
    return (
      <div className="font-mono bg-black text-white min-h-screen italic uppercase tracking-widest flex items-center justify-center">
        Carregando_Protocolos...
      </div>
    )
  }
  ...
  const handleQuest = (
    q: { id: string; title: string; reward: number; xp: number; stats: Partial<Record<string, number>>; type: string; amount: number },
    type: 'daily' | 'system',
  ) => {
          ...
          {evolutionQuests.map(q => {
            ...
            return (
              <div key={q.id} className={`relative bg-slate-950 border p-5 transition-all duration-300 ${isDone ? 'border-green-500/50 bg-green-950/10' : 'border-yellow-900/30'}`}>
                ...
                {!isDone && (
                  <button onClick={() => handleQuest(q, 'daily')}
                    className="w-full border border-yellow-500 text-yellow-500 py-2 text-[10px] font-black hover:bg-yellow-500 hover:text-black transition-colors uppercase">
                    Executar Protocolo
                  </button>
                )}
              </div>
          ...
          {asCincoMissoes.map(q => {
            ...
            return (
              <div key={q.id} className={`relative bg-slate-950/40 border p-4 flex justify-between items-center transition-all ${isDone ? 'border-green-900/40 bg-green-900/5' : 'border-cyan-900/20'}`}>
                ...
                {isDone ? (
                  <span className="text-green-500 font-black text-[10px] italic uppercase tracking-tighter flex items-center gap-1">
                    <Check size={12} /> FEITA
                  </span>
                ) : (
                  <button onClick={() => handleQuest(q, 'system')}
                    className="px-6 py-2 border border-cyan-500 text-cyan-500 text-[9px] font-black hover:bg-cyan-500 hover:text-black uppercase transition-colors">
                    Confirmar
                  </button>
                )}
              </div>
