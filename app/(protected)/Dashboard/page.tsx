...
function StatMiniCard({ label, value, sub, color }: { label: string; value: string | number; sub: string; color: string }) {
  return (
...
function ResourceProgress({ label, value, percent, icon }: { label: string; value: string; percent: number; icon: React.ReactNode }) {
    ...
    <div className="p-4 md:p-8 font-mono bg-black text-white min-h-screen pb-40">
      ...
      <header className="mb-10 border-b border-cyan-900/40 pb-6 flex justify-between items-end">
        ...
        <div className="text-right hidden md:block">
          ...
          <p className="text-[9px] text-slate-700 font-bold">LV.{level} | {xp.toLocaleString()} XP</p>
        </div>
      </header>
      ...
      <div className="flex flex-wrap gap-2 mb-8 border-y border-slate-900 py-4 overflow-x-auto">
        {MESES.map((mes, index) => (
          <button key={mes} onClick={() => setSelectedMonth(index)}
            className={`px-3 py-2 text-[9px] font-black border ${
              selectedMonth === index
                ? 'bg-cyan-500 text-black border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.4)]'
                : 'text-slate-600 border-slate-900'
            }`}>
            {mes}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="space-y-3">
            ...
            <StatMiniCard label="Missões Concluídas" value={log.tasks}                       sub="UN"  color="text-white"       />
            <StatMiniCard label="XP Adquirido"       value={`+${log.xpGain.toLocaleString()}`} sub="PTS" color="text-green-500"  />
            <StatMiniCard label="Gold Obtido"        value={`${log.goldGain.toLocaleString()}`} sub="G"   color="text-yellow-500" />
          </div>
              ...
              <div className="space-y-2">
                ...
                {routines?.map(r => (
                  <div key={r.id}
                    ...
                    <CheckCircle2 size={14} className={r.completedToday ? 'text-emerald-500' : 'text-slate-800'} />
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-slate-950 border border-slate-900 p-6 md:p-8">
            <h2 className="text-[10px] font-black uppercase mb-8 flex items-center gap-2 text-slate-400 tracking-widest">
              <Activity size={14} className="text-cyan-500" /> Monitor_de_Evolução: {MESES[selectedMonth]}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
              <ResourceProgress label="Hidratação"   value={`${(log.water||0).toFixed(1)}L`}    percent={((log.water||0)/60)*100}     icon={<Droplets size={12}/>}   />
              <ResourceProgress label="Flexões"      value={`${log.pushups||0} reps`}            percent={((log.pushups||0)/1000)*100} icon={<Dumbbell size={12}/>}   />
              <ResourceProgress label="Agachamentos" value={`${log.squats||0} reps`}             percent={((log.squats||0)/1000)*100}  icon={<Zap size={12}/>}        />
              <ResourceProgress label="Leitura"      value={`${log.reading||0} min`}             percent={((log.reading||0)/600)*100}  icon={<BookOpen size={12}/>}   />
                ...
                {systemQuests.map(q => (
                  <div key={q.id}
                    ...
                    className={`flex items-center justify-between p-4 border transition-all cursor-pointer ${
                      q.completed ? 'opacity-30 border-slate-900 bg-black' : 'bg-black border-slate-800 hover:border-blue-500'
                    }`}>
                    <span className="text-[10px] font-bold text-slate-300">{q.title}</span>
